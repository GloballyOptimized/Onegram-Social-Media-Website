...
from core.models import Profile ,Post ,Liked_Post,FollowersCount
...
def index(request):
    ...
    return render(request ,'index.html',{'user_profile':user_profile , 'posts' : feed_list , 'user_object' : user_object ,'suggestions_username_profile_lists':suggestions_username_profile_lists[:4]})
...
def like_post(request):
    ...
    like_filter = Liked_Post.objects.filter(post_id = post_id , username = username).first()

    if like_filter == None:
        new_like = Liked_Post.objects.create(post_id = post_id , username = username)
        new_like.save()
